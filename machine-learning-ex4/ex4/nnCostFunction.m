...
% Reshape nn_p arams back into the parameters Theta1 and Theta2, the weight matrices
...
for i=1:num_labels
	yi = find(y==i);
	Y(yi,:) = repmat(E(i,:),size(yi,1),1);
end


% 这个模型中间有一层隐藏层，但是J中的H(theta)对应的是最终的输出结果
X = [ones(m,1) X];
a2 = sigmoid(X * Theta1');
...
RTheta1 = sum(sum(temp1.^2));
RTheta2 = sum(sum(temp2.^2));


J = sum(sum(J1)) / m + lambda/(2*m)*(RTheta1+RTheta2);
...
for t = 1:m
	...
	err2 = Theta2' * err3;                % err_2有26行！！！
   	err2 = err2(2:end) .* sigmoidGradient(rz2);  

   	delta_2 = delta_2 + err3 * ra2';
   	delta_1 = delta_1 + err2 * ra1';
end
